...
vector<vector<string>> suggestedProductsM1(vector<string>& products, string searchWord)
{
    sort(products.begin(), products.end());
    string queryStr;
    auto lastItr = products.begin();
    vector<vector<string> > res;

    for (auto& ch :searchWord)
    {
        queryStr += ch;
        auto findItr = lower_bound(lastItr, products.end(), queryStr);
        vector<string> selects;
        for (int i = 0; i < 3; ++i)
        {
            if ((findItr + i) < products.end() && (*(findItr + i)).find(queryStr) == 0)
            {
                selects.push_back(*(findItr + i));
            }
        }
...
void addWord(Tree* root, const string& word)
{
    Tree* cur = root;
    for (auto& ch : word)
    {
